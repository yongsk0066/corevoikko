...
export { VoikkoError, WasmLoadError, DictionaryLoadError } from './wasm-loader.js';

// ── Internal WASM result types ──────────────────────────────────

interface WasmGrammarError {
  errorCode: number;
  startPos: number;
  errorLen: number;
  suggestions: string[];
 ...
 * @example
 * ```typescript
 * // Zero-config — works in both Node.js and Browser
...
export class Voikko {
  ...
  /** Release all resources. Safe to call multiple times (no-op after first). */
  terminate(): void {
   ...
   */
  grammarErrors(text: string): GrammarError[] {
    this.ensureActive();
    const raw: WasmGrammarError[] = this.#handle!.grammarErrorsFromText(text);
    return raw.map((e) => ({
  ...
  tokens(text: string): Token[] {
    this.ensureActive();
    const raw: WasmToken[] = this.#handle!.tokens(text);
    return raw.map((t) => ({
  ...
  sentences(text: string): Sentence[] {
    this.ensureActive();
    const raw: WasmSentence[] = this.#handle!.sentences(text);
  ...
  setIgnoreDot(value: boolean): void { this.ensureActive(); this.#handle!.setIgnoreDot(value); }

  /** Ignore words containing numbers. Default: false */
  setIgnoreNumbers(value: boolean): void { this.ensureActive(); this.#handle!.setIgnoreNumbers(value); }

  /** Accept words written entirely in uppercase without checking. Default: false */
  setIgnoreUppercase(value: boolean): void { this.ensureActive(); this.#handle!.setIgnoreUppercase(value); }

  /** Accept words when the first letter is uppercase. Default: true */
  setAcceptFirstUppercase(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptFirstUppercase(value); }

  /** Accept words when all letters are uppercase (still checked). Default: true */
  setAcceptAllUppercase(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptAllUppercase(value); }

  /** Ignore non-words such as URLs and email addresses. Default: true */
  setIgnoreNonwords(value: boolean): void { this.ensureActive(); this.#handle!.setIgnoreNonwords(value); }

  /** Allow some extra hyphens in words. Default: false */
  setAcceptExtraHyphens(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptExtraHyphens(value); }

  /** Accept missing hyphens at word boundaries. Default: false */
  setAcceptMissingHyphens(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptMissingHyphens(value); }

  /** Accept incomplete sentences in titles/headings. Default: false */
  setAcceptTitlesInGc(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptTitlesInGc(value); }

  /** Accept incomplete sentences at paragraph end. Default: false */
  setAcceptUnfinishedParagraphsInGc(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptUnfinishedParagraphsInGc(value); }

  /** Accept paragraphs valid as bulleted list items. Default: false */
  setAcceptBulletedListsInGc(value: boolean): void { this.ensureActive(); this.#handle!.setAcceptBulletedListsInGc(value); }

  /** Skip ugly but correct hyphenation positions. Default: false */
  setNoUglyHyphenation(value: boolean): void { this.ensureActive(); this.#handle!.setNoUglyHyphenation(value); }

  /** Hyphenate unknown words. Default: true */
  setHyphenateUnknownWords(value: boolean): void { this.ensureActive(); this.#handle!.setHyphenateUnknownWords(value); }

  /** Minimum length for words that may be hyphenated. Default: 2 */
  setMinHyphenatedWordLength(value: number): void { this.ensureActive(); this.#handle!.setMinHyphenatedWordLength(value); }
